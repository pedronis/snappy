...
func (c *Configurator) RcvDevice(b []byte) error {
	if c.nonce1 == nil {
		return fmt.Errorf("nonce1 must have been sent")
	}
	sig, err := jose.ParseSigned(string(b))
	if err != nil {
		// XXX maybe fatal instead
		return fmt.Errorf("can't parse signed device: %v", err)
	}
	if len(sig.Signatures) != 1 || sig.Signatures[0].Protected.ExtraHeaders["m"] != "device" {
		return fmt.Errorf("invalid device message")
	}
	var dev device
	if c.onbDevKey == nil {
		...
		if !ok && pk.Curve != elliptic.P256() {
			return fmt.Errorf("device didn't advertise expected public key")
		}
		c.onbDevKey = pk
	} else {
	...
	if !bytes.Equal(c.nonce1, dev.Nonce1) {
		return fmt.Errorf("device didn't sign correct nonce")
	}
	if len(dev.Nonce2) != nonceSize {
		return fmt.Errorf("nonce2 has the wrong size")
	}
	c.nonce2 = dev.Nonce2
	return nil
}
...
func (c *Configurator) SessionSetup() ([]byte, error) {
	if c.onbSecret == nil {
		return nil, fmt.Errorf("onboarding secret must be set")
	}
	if c.nonce2 == nil {
		return nil, fmt.Errorf("nonce2 must have been received")
	}
	// generate session key
	sek := make([]byte, sessKeySize)
	if _, err := jose.RandReader.Read(sek); err != nil {
		return nil, fmt.Errorf("cannot generate session key")
	}
	c.sek = sek
	...
	if err != nil {
		return nil, fmt.Errorf("can't serialize session")
	}
	signed, err := cftorSign.Sign(b)
	if err != nil {
		return nil, fmt.Errorf("can't hash session")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("can't prepare session encryption")
	}
	encrypted, err := enc.Encrypt([]byte(signed.FullSerialize()))
	if err != nil {
		return nil, fmt.Errorf("can't encrypt session")
	}
...
func (c *Configurator) sessDecrypt(b []byte, m string) ([]byte, error) {
	if c.sek == nil {
		return nil, fmt.Errorf("session key must be established")
	}
...
func (c *Configurator) RcvReady(b []byte) (map[string]interface{}, error) {
	...
	if err := json.Unmarshal(b, &deviceReady); err != nil {
		return nil, fmt.Errorf("can't deserialize ready")
	}
...
func (c *Configurator) Cfg(directives map[string]interface{}) ([]byte, error) {
	if !c.ready {
		return nil, fmt.Errorf("must have received ready")
	}
	if c.cfgEnc == nil {
	...
	if err != nil {
		return nil, fmt.Errorf("can't encrypt cfg")
	}
	return []byte(encrypted.FullSerialize()), nil
}
